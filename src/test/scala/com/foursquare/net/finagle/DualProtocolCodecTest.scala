...
package com.foursquare.net.finagle
...
class DualProtocolCodecTest {
  @Test
  def testRespondOnSamePort() {
    // Make the dual services for testing.

    val processor = new DualService.FutureIface {
      def add(x: Int, y: Int): Future[Int] = Future.value(x + y)
      def trace(): Future[String] = Future.value("NOP")
    }

    val thriftService = new DualService.FinagledService(processor, new TBinaryProtocol.Factory())
    ...
    val thriftClient = new DualService.FinagledClient(thriftServiceClient, new TBinaryProtocol.Factory())
    ...
    try {
      def checkThrift(x: Int, y: Int) {
        ...
        A.assertEquals(remoteAnswer, expectedAnswer)
      }
      ...
      def checkHttp(path: String, expectedStatus: HttpResponseStatus) {
        ...
        A.assertEquals(response.status, expectedStatus)
      }
